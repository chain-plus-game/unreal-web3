...
namespace
{
  ...
  const uint64 XorMasks[KeccakRounds] =
  {
  ...
  inline uint64 rotateLeft(uint64 x, uint8 numBits)
  {
    return (x << numBits) | (x >> (64 - numBits));
  }

  /// convert litte vs big endian
  inline uint64 swap(uint64 x)
  {
...
#define LITTLEENDIAN(x) (x)
  ...
  {
    // Theta
      uint64 coefficients[5];
    for (unsigned int i = 0; i < 5; i++)
      coefficients[i] = m_hash[i] ^ m_hash[i + 5] ^ m_hash[i + 10] ^ m_hash[i + 15] ^ m_hash[i + 20];

    for (unsigned int i = 0; i < 5; i++)
    {
        uint64 one = coefficients[mod5(i + 4)] ^ rotateLeft(coefficients[mod5(i + 1)], 1);
      m_hash[i     ] ^= one;
      m_hash[i +  5] ^= one;
      m_hash[i + 10] ^= one;
      m_hash[i + 15] ^= one;
...
FString Keccak::getHash()
{
  // save hash state
  uint64 oldHash[StateSize];
...
FString Keccak::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
FString Keccak::operator()(const FString& text)
{
  reset();
  add(*text, text.GetAllocatedSize());
  return getHash();
}